...
| tipo | String(20) NOT NULL | enum: ligacao/email/whatsapp/reuniao/linkedin_conexao/linkedin_mensagem/tarefa_manual/followup |
| responsavel_id | UUID (FK users) NOT NULL, index | |
...
| tipo | String(20) NOT NULL | ligacao/email/whatsapp/linkedin_conexao/linkedin_mensagem/tarefa_manual/followup |
| template_id | UUID (FK email_templates) nullable | quando tipo = email (envio automático via Graph) |
| instrucoes | Text nullable | roteiro/instruções para ligação/whatsapp/linkedin_conexao/linkedin_mensagem/tarefa_manual (ex.: "incluir produto") |
...
Fluxo: enroll de uma empresa/contato/negócio em uma `sequence` → Celery Beat varre `sequence_enrollments` diariamente → para cada step vencido (`dia_offset`), tenta enviar de verdade (e-mail via Microsoft Graph; WhatsApp via Twilio, quando configurado e o modelo tiver um Content Template aprovado pela Meta) e, se não der, cria uma `Task` (ligação/e-mail/whatsapp/linkedin/follow-up) atribuída ao responsável → avança `step_atual`. **LinkedIn (conexão/mensagem) nunca envia automático** — decisão deliberada, não pendência técnica: não existe API oficial que cubra prospecção fria sem violar os Termos de Uso do LinkedIn (risco de banimento de conta); essas etapas sempre geram `Task` com o texto pronto pra copiar. Regra "pausar quando houver resposta" **depende de um sinal de resposta recebida**, que só existe se houver integração de caixa de entrada (**Microsoft 365/Graph** — a GD Conecta usa essa suíte corporativamente, ver §9.2 — não Gmail/Outlook genérico) — ou seja, a integração de e-mail é pré-requisito técnico dessa regra, não pode ser implementada isoladamente. Clonar sequência e reiniciar são operações de service simples sobre `sequences`/`sequence_steps`. Ver `app/services/sequence_dispatch.py` e `app/services/twilio_whatsapp.py`.
...
| **Fase 2** | Sequências de Tarefas, Cadência de E-mails, Workflows, Modelos de e-mail, integração **Microsoft 365/Graph** (necessária para "pausar por resposta") e envio automático de **WhatsApp** (Twilio) — implementados. LinkedIn permanece deliberadamente manual (sem API oficial segura pra prospecção fria, ver §7.5) | ✅ Concluída (WhatsApp real depende de `TWILIO_WHATSAPP_FROM` + templates aprovados pela Meta em produção — sem isso, cai no fallback de Tarefa manual) | Conta Microsoft 365 conectada (Graph API); verificação Meta Business Manager + WhatsApp Sender aprovado (Twilio como BSP) |
